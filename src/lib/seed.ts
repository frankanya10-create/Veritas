const BIP39_WORDS = [
  ...
  "uncover","under","undo","unfair","unfold","unhappy","uniform","union","unique","unit",
  ...
  "wine","wing","wink","winner","winter","wire","wisdom","wish","witness","wolf",
  "woman","wonder","wood","wool","word","work","world","worry","worth","wrap",
  "wreck","wrestle","wrist","write","wrong","yard","year","yellow","you","young",
  "youth","zebra","zero","zone","zoo"
];

export function generateSeedWords(count: 12 | 24 = 24): string[] {
  const seed = new Array(count);
  for (let i = 0; i < count; i++) {
    const idx = Math.floor(Math.random() * BIP39_WORDS.length);
    seed[i] = BIP39_WORDS[idx];
  }
  return seed;
}

export async function hashSeed(words: string[]): Promise<string> {
...
export function downloadSeed(words: string[], format: "txt" | "json" = "txt"): void {
  let content: string;
  let filename: string;
  let mimeType: string;
  ...
  if (format === "json") {
    content = JSON.stringify({
      ...
      warning: "KEEP THIS SECRET. Anyone with these words can recover your workspace.",
    }, null, 2);
    filename = "veritas-recovery-seed.json";
    mimeType = "application/json";
  } else {
    content = [
      "VERITAS RECOVERY SEED",
      "=====================",
      "KEEP THIS SECRET. Anyone with these words can recover your workspace.",
      "",
      words.map((w, i) => `${String(i + 1).padStart(2, " ")}. ${w}`).join("\n"),
      "",
      `Generated: ${new Date().toISOString()}`,
    ].join("\n");
    filename = "veritas-recovery-seed.txt";
    mimeType = "text/plain";
  }
